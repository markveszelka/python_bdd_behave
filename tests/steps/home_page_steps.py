...
def step_impl(context):
    assert Texts.HOME_PAGE_TEXT in context.home_page.driver.title, \
        f"Expected title '{Texts.HOME_PAGE_TEXT}' not found in page title."


@then(r'the page with "(.+)" is opened')
def step_impl(context, url):
    expected_url = URLs.HOME_PAGE_URL + url
